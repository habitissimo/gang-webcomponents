...
use Gang\WebComponents\Contracts\TagInterface;
use Gang\WebComponents\Parser\Nodes\Fragment;
use Gang\WebComponents\Parser\Nodes\Tag;
use Gang\WebComponents\Parser\Nodes\WebComponent;

/**
 * Class that parses all the string (HTML) and returns a collection of
 * Fragments (non renderable content) and WebComponents (objects to be processed
 * for further rendering).
 *
 * Acts like a state machine.
 * Uses a SAX like parser for HTML.
 */
class Parser
{
    private $deep = 0;
    ...
    public function _voidElementHandler($parser, $name, $attrs): void
    {
        ...
        $this->buffer->appendOpeningXmlTag($name, $attrs, true);

        if ($opened || !$this->insideComponent()) {
